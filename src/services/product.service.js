...
class ProductServices {
  async addProduct(productData) {
    try {
      if (
        ...
        throw new Error(
          'Es necesario completar todos los campos de información del producto.'
        );
      }

      const productExist = await ProductRepository.getProductByCode({
  ...
  async updateProduct(id, productData) {
    try {
      const update = await ProductRepository.updateProductById(id, productData);
      ...
      throw new Error('No es posible actualizar el producto. ' + error.message);
    }
  }
  ...
  async paginateProducts(filter, options) {
    try {
      const productsPaginated = await ProductRepository.productsPaginate(
        filter,
        options
      );
